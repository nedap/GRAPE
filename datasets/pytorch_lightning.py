...
class PartNetDataModule(LightningDataModule):
    ...
    def train_dataloader(self):
        ...
        return DataLoader(
            ...
            shuffle=True, 
            drop_last=True, #TODO: self.cfg.others.subset == 'train', 
...
class GNNDataModule(LightningDataModule):
    def __init__(self, args, cfg, k_fold: bool = False) -> None:
        ...
        self.full_dataset = build_dataset_from_cfg(self.cfg.dataset.train) if not self.k_fold else [] # in case of k_fold we dont need the full dataset
        ...
        if not self.k_fold:
            ...
            self.train_dataset, self.val_dataset, self.test_dataset = random_split(self.full_dataset, [train_size, val_size, test_size])
    ...
    def setup_kfold(self, fold=None) -> None:
        ...
        available_folds = len([f for f in Path(self.cfg.dataset.train.DATA_PATH).iterdir() if f.is_dir() and f.name.startswith('fold_')])
